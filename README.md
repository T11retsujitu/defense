# 防衛調達データベース (PoC)

防衛省・自衛隊が分散して公表する**調達契約情報**を自動取得・正規化し、
企業・年度・カテゴリ・調達機関横断で検索・集計できる産業データベースのProof of Concept。
現在は防衛装備庁の**中央調達(全量)**と北関東防衛局の**地方調達(物品・役務、試験収録)**を収録。

軍事ファンサイトではなく、EDINET系データサービス・政府統計ダッシュボードに近い
ニュートラルな産業データベースを目指す。

## 現状 (PoC完了時点)

- 実データ収録: 令和5〜8年度の公表ファイル**78件(公表済み全月)** / 契約**21,208件(約16.3兆円)** / 契約相手方1,203
- parse失敗 0件(注記行はrawのみ保持)、企業未解決 6件(小規模事業者のみ)
- カテゴリは**二軸**(装備分野×契約目的)だが、成熟度は軸ごとに大きく異なる:
  - **装備分野**: ルール分類済みが**金額ベース96%**(件数ベース72%)。未分類には
    理由コード(insufficient_context / cross_domain / manual_review_pending)を付与し、
    改善対象(rule gap)と本質的に分類困難なものを区別
  - **契約目的**: 研究開発/改修・能力向上/調査・検討/維持整備/役務/燃料・消耗品/産業基盤 を判定するが、
    ルール分類済みは**金額ベース39%・件数ベース30%にとどまる**。未一致は既定値「物品取得・その他」
    (nature_rule_matched=0で仮定と明示)であり、**目的軸は現時点では部分的な分類**。
    サイト上でも各所にカバレッジと「既定値」の印を表示している
- **地方調達の収録を開始**(地方調達PoC): 北関東防衛局の物品・役務(財計第2017号様式PDF)を
  中央調達と同一スキーマへ正規化。法人番号による名寄せが機関横断で機能する。
  防衛省の公表対象調達 約8.4兆円(令和5年度)のうち中央調達5.6兆円(67%)は全量収録済み、
  地方調達2.8兆円(33%)は北関東防衛局分のみ([docs/regional-survey.md](docs/regional-survey.md))
- **市場区分(scope)軸を追加**: 地方調達には基地運営の一般契約(食料・清掃等)が多く、
  装備契約と単純合算すると防衛産業の像が歪むため、
  任務装備/任務支援/基地インフラ/基地運営/一般物品・事務/糧食・食料 を区別する
- 契約相手方を区分管理: 民間企業 12.9兆円 / 外国政府等(FMS) 3.3兆円 / 公的機関 0.06兆円。
  **企業ランキングは民間企業のみ**(FMS等は契約相手方ランキングに分離)
- 完全一致の重複行62件(41億円)は`suspected_duplicate`フラグで可視化(削除しない)
- 未公表月は404マーカー(TTL付き: 進行年度3日/過去年度90日)で管理、
  取得済みファイルも約30日毎にETag/Last-Modifiedで訂正を検知
- 一次データ調査の詳細: [docs/data-survey.md](docs/data-survey.md)

## 集計の意味論 (重要)

- **二軸分類**: 「何を調達したか(装備分野)」と「何のための契約か(契約目的)」を独立に分類。
  例: 「極超音速誘導弾等の生産準備役務」= 誘導弾・ミサイル × 役務・サービス
- **rule_matchedはルール一致のブール値**であり、分類精度の確信度ではない
- **デフォルト表示年度は全12か月収録済みの最新年度**。進行中年度は「4〜6月累計」のように
  期間を明示し、前年比較は**前年同期(同じ暦月)**で行う
- 契約金額は防衛関連売上高ではない(全ページに注記)
- **複数年度契約は締結年度に全額計上**。公表ファイルに履行期間の列がないため機械判定・年割換算は
  不可能であり、代替としてランキングには総額の過半が単一契約による相手方に
  「単一契約集中」の印を表示する(例: 令和7年度4位の衛星コンステレーション事業SPCは
  履行期間5年超の約2,831億円を1件で計上)

## 技術構成

| 層 | 技術 | 選定理由 |
|---|---|---|
| ETL | Python 3 + openpyxl | 公表データがxlsx。標準的で長期維持しやすい |
| DB | SQLite | 全期間でも数万行規模。ファイル1個で運用でき、維持費ゼロ。将来Postgresへ移行可能なSQL |
| Web | Flask + Jinja2 (SSR) | ページ数が少なくSSRで十分。SEOに必要なHTMLを直接返す。将来は静的書き出し/CDN前段も容易 |
| テスト | pytest | パーサ・正規化の回帰防止 |

フロントエンドフレームワーク・ビルドチェーンは不使用(依存最小・表示速度優先)。

## セットアップ

```bash
pip install -r requirements.txt
```

## データ取得・更新

```bash
# リポジトリ同梱の取得済みファイル(data/raw/)だけでDBを構築(ネットワーク不要)
make db

# 公表元から新規ファイルを取得して更新(リクエスト間隔3秒、取得済みはキャッシュ利用)
python3 -m etl.run_import --years 2023 2024 2025 2026

# ソース系統を絞る(atla=中央調達 / n-kanto=北関東防衛局)
python3 -m etl.run_import --years 2025 2026 --sources n-kanto
```

- 取得結果(成功/失敗/parse失敗/未解決企業/未分類件数)は標準出力と `import_jobs` テーブルに記録される。
- 未公表月の404は `.404` マーカーで記録し、再アクセスしない。
- **防衛省サーバーへの高頻度アクセスは行わないこと。**

## Web起動

```bash
make web   # = python3 -m web.app → http://127.0.0.1:5000
```

ページ: `/` `/contracts/` `/contracts/<id>/` `/companies/<slug>/` `/categories/<slug>/`
`/rankings/companies/` `/sources/` `/methodology/`

## テスト

```bash
make test  # = python3 -m pytest tests/
```

金額(カンマ・注記・億万表記・全角)、日付(Excel型・和暦・注記)、法人名(（株）展開・全角半角)、
列順入れ替え・列欠落などのケースを含む。

## DB構造

`etl/schema.sql` 参照。要点:

- **raw層とnormalized層の分離**: `raw_contracts` は公表ファイルの原文を全列保持。
  `contracts` は正規化済み。正規化ロジック改修後は raw から全再生成できる
  (`normalization_version` で世代管理)。
- **企業同定**: `companies` + `company_aliases`。第一キーは公表ファイル記載の**法人番号**。
  法人番号のないFMS(米軍各省)等はシード辞書で解決し `entity_type` で区別。
  文字列類似だけで別法人を統合しない。
- **出典追跡**: 全契約が `sources` (取得元URL・SHA-256・取得日時・ライセンス)に紐づく。
- **品質可視化**: parse失敗・未解決・未分類は `normalization_status` / `normalization_flags`
  に記録し、破棄しない。

## データソースと利用規約

- 出典: 防衛装備庁「[契約に係る情報の公表（中央調達分）](https://www.mod.go.jp/atla/souhon/supply/jisseki/rakusatu/index.html)」を加工して作成
- ライセンス: [公共データ利用規約(第1.0版)(PDL1.0)](https://www.mod.go.jp/j/info/contents.html) — CC BY 4.0互換、出典記載必須
- 収録範囲は**中央調達のみ**(地方調達・非公表契約を含まない)。
- **契約金額は企業の防衛関連売上高ではない。** この注意はサイト全ページに表示している。

## デプロイ

小型VPS + Docker(gunicorn) + Caddy を採用。詳細と運用手順は
[docs/deployment.md](docs/deployment.md) 参照。ローカル検証:

```bash
docker compose up --build   # http://localhost:8000
```

## 関連ドキュメント

- [docs/data-survey.md](docs/data-survey.md) — 一次データ調査(中央調達xlsx)
- [docs/regional-survey.md](docs/regional-survey.md) — 地方調達の調査と収録方針(北関東防衛局PoC・WARP経由のURL発見)
- [docs/honbu-pdf-survey.md](docs/honbu-pdf-survey.md) — 本部一般調達PDFの調査と対応方針(次期スコープ)
- [docs/deployment.md](docs/deployment.md) — デプロイ構成

## ディレクトリ

```
etl/        取得・パース・正規化・投入 (schema.sql含む)
web/        Flaskアプリ + テンプレート
data/raw/   取得した公表ファイル(原本キャッシュ、コミット対象)
data/db/    SQLite (生成物、gitignore)
tests/      パーサ・正規化・分類のテスト
docs/       一次データ調査結果
```
