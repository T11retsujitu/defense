...
- カテゴリは**二軸**(装備分野×契約目的)。装備分野の未分類は金額ベース約4%で、
  理由コード(insufficient_context / cross_domain / manual_review_pending)により
  改善対象(rule gap)と本質的に分類困難なものを区別
- 契約目的は 研究開発/改修・能力向上/調査・検討/維持整備/役務/燃料・消耗品/産業基盤 を判定し、
  未一致は既定値「物品取得・その他」(nature_rule_matched=0で仮定と明示)
